...
int main() {
    ...
    const char *outputDir_b = "output/output_b_loop";   // Директория с результатами обработки программы B
    const char *logDir_b = "logs/logs_b_loop";          // Директория с логами программы B
    ...
    for(int img = 0; img < sizeof(imageFiles) / sizeof(imageFiles[0]); img++) {
        inputImage = imageFiles[img];
